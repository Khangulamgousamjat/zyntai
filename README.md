...
1. **Clone the repository**:
   ```bash
   git clone https://github.com/Khangulamgousamjat/zyntai.git
   cd zyntai
   ```
...
To guarantee zero regressions and production stability:

```bash
# 1. Validate TypeScript strict types across the whole codebase
npm run typecheck
...
The compiled assets will be output to the `/dist` directory, tree-shaken and minified for peak Web Vitals scores.
...
This repository includes a pre-configured [`vercel.json`](./vercel.json) ensuring all SPA routes (`/pricing`, `/faq`, `/about`, `/contact`, `/login`, `/signup`) route seamlessly to `index.html`:
...
Alternatively, import the repository directly into the [Vercel Dashboard](https://vercel.com/new). Settings will be detected automatically:
