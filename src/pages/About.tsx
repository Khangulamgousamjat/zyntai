  ...
  return (
    <div className="relative">
      ...
      <section className="relative overflow-hidden pt-40 pb-16 sm:pt-48 sm:pb-20">
        <div className="absolute inset-0 grid-lines [mask-image:radial-gradient(70%_60%_at_50%_20%,black,transparent)] pointer-events-none" />
        ...
        <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
          ...
          >
            Built for modern <em className="font-accent font-normal italic text-violet-400">businesses.</em>
          </motion.h1>
          <Reveal delay={0.2}>
      ...
      <section className="relative py-16 sm:py-24">
        <div className="mx-auto grid max-w-7xl gap-14 px-5 sm:px-8 lg:grid-cols-[1fr_1.5fr] lg:gap-24">
          <div className="lg:sticky lg:top-32 lg:self-start">
            <Reveal>
              <blockquote className="glass relative overflow-hidden rounded-[1.25rem] border border-char-700/60 p-8 shadow-xl">
                <span aria-hidden className="font-accent absolute -top-2 right-5 select-none text-[6rem] italic leading-none text-violet-500/10">
                  “
                </span>
                ...
                <footer className="mt-5 flex items-center gap-3">
                  <span className="grid h-10 w-10 place-items-center rounded-full bg-violet-600 text-[0.7rem] font-bold text-white shadow-sm">
                    RH
                  </span>
                  <div>
            ...
            <Reveal delay={0.22}>
              <div className="flex flex-wrap gap-3 pt-2">
                {["Remote-first, 14 countries", "Bootstrapped until 2025", "Profitable & growing"].map((c) => (
                  <span key={c} className="rounded-full border border-char-700 bg-char-800/60 px-4 py-2 text-[0.8rem] font-medium text-zinc-300">
                    {c}
                  </span>
                ))}
              </div>
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-3">
            {VALUES.map((v, i) => (
              <Reveal key={v.title} delay={0.1 * i} y={44} className={i === 1 ? "md:translate-y-8" : ""}>
                <article className="group glass relative h-full overflow-hidden rounded-[1.25rem] border border-char-700/60 p-8 transition-all duration-300 hover:-translate-y-1 hover:border-violet-500/40 hover:shadow-[0_20px_50px_-20px_rgba(0,0,0,0.5)]">
                  <span aria-hidden className="font-display absolute right-6 top-4 select-none text-[4.2rem] font-bold leading-none text-white/[0.045] transition-colors duration-300 group-hover:text-violet-400/10">
                    {v.n}
                  </span>
                  <span className="grid h-12 w-12 place-items-center rounded-xl border border-violet-500/25 bg-violet-500/10 text-violet-400 transition-transform duration-300 group-hover:scale-105">
      ...
      <section className="relative py-16 sm:py-24">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <motion.div
            ...
            className="liquid-glass grid grid-cols-2 overflow-hidden rounded-[1.25rem] border border-char-700/60 lg:grid-cols-4"
          >
            {ABOUT_STATS.map((s, i) => (
              <div
                key={s.label}
                className={`group relative px-6 py-9 text-center transition-colors duration-300 hover:bg-violet-500/[0.04] sm:px-8 ${
                  i !== 0 ? "border-l border-char-700/60 max-lg:[&:nth-child(3)]:border-l-0" : ""
                } ${i >= 2 ? "max-lg:border-t max-lg:border-char-700/60" : ""}`}
              >
                <p className="font-display text-4xl font-bold tracking-tight text-zinc-50 transition-colors duration-300 group-hover:text-violet-400 sm:text-[2.7rem]">
      ...
      <section className="relative px-5 py-16 sm:px-8 sm:py-24">
        <motion.div
          ...
          viewport={{ once: true, margin: "-60px" }}
          transition={{ duration: 0.9, ease: EASE as unknown as number[] }}
          className="relative mx-auto max-w-6xl overflow-hidden rounded-[1.5rem] border border-char-700 bg-char-900/90 px-6 py-16 text-center shadow-2xl sm:px-16 sm:py-20"
        >
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(124,58,237,0.12),transparent_65%)]" />
          <h2 className="font-display relative mx-auto max-w-2xl text-balance text-3xl font-bold leading-[1.06] tracking-tight text-zinc-50 sm:text-5xl">
            Ready to meet <em className="font-accent font-normal italic text-violet-400">Zyntai?</em>
          </h2>
          <p className="relative mx-auto mt-4 max-w-lg text-lg font-normal text-zinc-400">
            See it on your business in a free 20-minute demo.
          </p>
          <Link to="/contact" className="btn btn-primary group relative mt-8 px-8 py-4 text-[1rem]">
            Book a free demo
